...
export default function Grade5GreetingActivity() {
  ...
  useEffect(() => {
  const allIndexes = Array.from(greetingConversations.keys());
  ...
  useEffect(() => {
    ...
    const rawQuestion = greetingConversations[questionOrder[currentIndex]];
    const newProcessedData = getQuestionData(rawQuestion);
  ...
  const handleSubmit = useCallback(() => {
    ...
    filledBlanks.forEach((droppedWord, i) => {
      if (droppedWord === answers[i]) {
        correctCount++;
      }
    });
...
//     const sourceIndex = event.active.data.current?.sourceIndex;
...
const handleDragEnd = useCallback((event) => {
  ...
  const totalPossiblePoints = totalQuestions * answers.length;
  ...
  return (
    ...
    >
      <ActivityContainer>
        <ActivityHeader title={t("activities.grade5.greetingActivity.title")}
                             description={t("activities.grade5.greetingActivity.description")}/>
        <ScoreCounter
        ...
        <ReactAudioPlayer
          key={currentIndex}
          src={greetingConversations[questionOrder[currentIndex]].audio}
          controls
          autoPlay
          style={{
            width: "100%",
